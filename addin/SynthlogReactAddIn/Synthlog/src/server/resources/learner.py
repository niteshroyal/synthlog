...
class TaskManager:
    ...
    def get_suggested_tasks(self):
        ...
        if tacle_tasks is not None:
            task_pool.append(tacle_tasks.DetectTablesTask(self.state))
            task_pool.append(tacle_tasks.DetectBlocksTask(self.state))
            task_pool.append(tacle_tasks.TacleTask(self.state))

        if psyche is not None:
            task_pool.append(psyche.PsycheTask(self.state))

        task_pool.append(ResetTask.ResetTask(self.state))
        # TODO Add MERCS back
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--state", help="State id of the spreadsheet, retrieved from the state database"
    )
    parser.add_argument(
        "--dev",
        help="Execute in development mode (raise exception, pretty print json)",
        action="store_true",
    )
