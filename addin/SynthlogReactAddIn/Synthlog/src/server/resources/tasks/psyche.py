...
import tacle
import numpy as np
from psyche.predictor_strategy import FormulaPredictorStrategy, DefaultSingleChainingPredictorStrategy
from psyche.psyche import PsycheContext
from psyche.spreadsheet import Spreadsheet, CellAddress

from state_manager import State, Prediction, Coordinate, ConstraintConverter
from .task import BaseTask


class PsycheTask(BaseTask):
    predictor_strategies = [DefaultSingleChainingPredictorStrategy(), FormulaPredictorStrategy()]

    def context(self) -> PsycheContext:
        spreadsheet = Spreadsheet(None)
        spreadsheet.tables = [t["tacle_table"] for t in self.state.tables if "tacle_table" in t]
        return PsycheContext(spreadsheet, ConstraintConverter.get_constraints(self.state))

    def find_missing_cells(self) -> List[CellAddress]:
        spreadsheet = self.context().spreadsheet
        ...
        for table in spreadsheet.tables:
            ...
            for i in range(table_indices.shape[0]):
                cell_addresses.append(
                    spreadsheet.address(table, table_indices[i, 0].item(), table_indices[i, 1].item()))
    ...
    def do(self) -> State:
        ...
        spreadsheet = Spreadsheet(None)
        spreadsheet.tables = [t["tacle_table"] for t in self.state.tables if "tacle_table" in t]
        context = PsycheContext(spreadsheet, ConstraintConverter.get_constraints(self.state),
        ...
        for confidence, values in best_predictions:
            if values is not None:
                ...
                for cell, value in values.items():
                    predictions.append(Prediction(Coordinate(cell.abs_x, cell.abs_y), value, confidence, provenance))
