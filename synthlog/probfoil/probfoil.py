...
from synthlog.probfoil.language import TypeModeLanguage
from problog.util import init_logger
from problog.logic import Term
from synthlog.probfoil.rule import FOILRule
from synthlog.probfoil.learn import CandidateBeam, LearnEntail
...
from synthlog.probfoil.score import (
    rates,
    accuracy,
    m_estimate_relative,
    precision,
    recall,
    m_estimate_future_relative,
    significance,
    pvalue2chisquare,
)
...
class ProbFOIL(LearnEntail):
    def __init__(
        self, data, beam_size=5, logger="probfoil", m=1, l=None, p=None, **kwargs
    ):
        LearnEntail.__init__(
            self, data, TypeModeLanguage(**kwargs), logger=logger, **kwargs
        )
        ...
        getLogger(self._logger).info(
            "Positive weight (P): %.4f" % sum(self._scores_correct)
        )
        getLogger(self._logger).info(
            "Negative weight (N): %.4f"
    ...
    def best_rule(self, current):
        ...
        current_rule = FOILRule(
            target=self.target, previous=current, correct=self._scores_correct
        )
        ...
        try:
            ...
            while candidates:
                ...
                getLogger(self._logger).debug(
                    "Best rule so far: %s [%s]" % (best_rule, best_rule.score)
                )
                getLogger(self._logger).debug(
                    "Candidates for iteration %s:" % iteration
                )
                ...
                while candidates:
                    ...
                    getLogger(self._logger).log(
                        8,
                        "TO AVOID: %s => %s"
                        % (current_rule, current_rule.avoid_literals),
                    )
                    c_tp, c_fp, c_tn, c_fn = rates(current_rule)
                    if self._max_length and len(current_rule) >= self._max_length:
                        ...
                        for ref in self.language.refine(current_rule):
                            if (
                                ref in current_rule.avoid_literals
                            ):  # or ref.prototype in current_rule.avoid_literals:
                                getLogger(self._logger).log(
                                    8,
                                    "SKIPPED literal %s for rule %s"
                                    % (ref, current_rule),
                                )
                                continue
                            ...
                            if (
                                prev_tp > r_tp - 1e-8
                            ):  # new rule has no tp improvement over previous
                                getLogger(self._logger).log(
                                    8,
                                    "%s %s %s %s [REJECT coverage] %s"
                                    % (
                                        rule,
                                        rule.score,
                                        rates(rule),
                                ...
                                r_fp > c_fp - 1e-8
                            ):  # and not rule.has_new_variables():
                                # no fp eliminated and no new variables
                                getLogger(self._logger).log(
                                    8,
                                    "%s %s %s %s [REJECT noimprov] %s"
                                    % (
                                        rule,
                                        rule.score,
                                        rates(rule),
                                ...
                                if rule.score_cmp > best_rule.score_cmp:
                                    getLogger(self._logger).log(
                                        9,
                                        "BETTER RULE %s %s > %s"
                                        % (rule, rule.score_cmp, best_rule.score_cmp),
                                    )
                                    best_rule = rule
                            else:
                                if r_fp > c_fp - 1e-8:
    ...
    def learn(self):
        ...
        while True:
            ...
            getLogger(self._logger).info(
                "RULE LEARNED: %s %s" % (next_hypothesis, new_score)
            )
            ...
            if self.interrupted:
                break
            if (
                hypothesis.get_literal()
                and hypothesis.get_literal().functor == "_recursive"
            ):
...
class ProbFOIL2(ProbFOIL):
    ...
    def _compute_rule_score_slow(self, rule, future=False):
        ...
        if score_x > max_score:
            ...
            max_score = score_x
            if not future:
                getLogger("probfoil").log(
                    6, "%s: x=%s (%s %s) -> %s" % (rule, 1.0, tp_x, fp_x, score_x)
                )
        ...
        for p, l, u in data:
            ...
            if not future:
                getLogger("probfoil").log(
                    6,
                    "%s: x=%s (%s %s %s) (%s %s) -> %s"
                    % (rule, x, p, l, u, tp_x, fp_x, score_x),
                )
            if score_x > max_score:
                max_x = x
                max_tp = tp_x
        ...
        if not future:
            getLogger("probfoil").log(
                6,
                "%s: [BEST] x=%s (%s %s) -> %s"
                % (rule, max_x, max_tp, max_fp, max_score),
            )
            rule.max_x = max_x
            rule.max_tp = max_tp
            rule.max_fp = max_fp
    ...
    def _compute_rule_score_fast(self, rule, future=False):
        ...
        if not future:
            getLogger("probfoil").log(
                5,
                "%s: %s"
                % (rule, list(zip(rule.correct, scores_previous, rule.scores))),
            )
        ...
        for p, l, u in zip(rule.correct, scores_previous, rule.scores):
            ...
            if ds == 0:  # no improvement
                ...
                y = (p - l) / (
                    u - l
                )  # for x equal to this value, prediction == correct
        ...
        if values:
            ...
            if not future:
                getLogger("probfoil").log(
                    5,
                    "%s: %s"
                    % (rule, [map(lambda vv: round(vv, 3), vvv) for vvv in values]),
                )
            ...
            for y, p, l, u in values + [
                ...
                if y is None or prev_y is not None and y > prev_y:
                    ...
                    if not future:
                        getLogger("probfoil").log(
                            6, "%s: x=%s (%s %s) -> %s" % (rule, x, tp_x, fp_x, score_x)
                        )
        ...
        if not future:
            getLogger("probfoil").log(
                6, "%s: [BEST] x=%s (%s %s) -> %s" % (rule, max_x, tp_x, fp_x, score_x)
            )
            rule.max_x = max_x
            rule.max_tp = max_tp
            rule.max_fp = max_fp
...
def argparser():
    ...
    parser.add_argument(
        "-m", help="parameter m for m-estimate", type=float, default=argparse.SUPPRESS
    )
    parser.add_argument(
        "-b", "--beam-size", type=int, default=5, help="size of beam for beam search"
    )
    parser.add_argument(
        "-p",
        "--significance",
        type=float,
        default=None,
        help="rule significance threshold",
        dest="p",
    )
    parser.add_argument(
