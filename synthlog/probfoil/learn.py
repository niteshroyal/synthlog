...
class LearnEntail(object):
    ...
    def load(self, data):
        ...
        if example_mode and str(example_mode[0][0]) == "auto":
            types = self.language.get_argument_types(
                self._target.functor, self._target.arity
            )
    ...
    def _compute_scores_correct(self):
        ...
        result = self._data.evaluate(
            rule=None, functor=self.target.functor, arguments=self.examples
        )
...
class CandidateBeam(CandidateSet):
    ...
    def push(self, candidate):
        ...
        if (
            ...
            or candidate.score_cmp > self._bottom_score()
        ):
