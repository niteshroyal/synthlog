...
class TypeModeLanguage(BaseLanguage):
    ...
    def add_types(self, functor, argtypes):
        ...
        if key in self._types:
            raise ValueError(
                "A type definition already exists for '%s/%s'."
    ...
    def refine(self, rule):
        ...
        for functor, modestr in self._modes:
            ...
            for argmode, argtype in zip(modestr, types):
                if argmode == "+":
                    ...
                    arguments.append(
                        [Var("#")]
                    )  # what about adding a term a(X,X) where X is new?
                elif argmode == "c":
                    # Add a constant
        ...
        # 3) recursive
        if (
            self._allow_recursion and rule.previous.previous
        ):  # recursion in the first rule makes no sense
