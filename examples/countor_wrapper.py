...
def load_count_or_constraints(workbook_id, worksheet_id, *args, **kwargs):
    ...
    rows = engine.query(database, Term("countor_constraint", workbook_id, worksheet_id, None), subcall=True)
    return [(workbook_id, worksheet_id, Object(pickle.loads(row[2].value))) for row in rows]
...
def save_count_or_constraint(filename, workbook_id, worksheet_id, constraint):
    ...
    types = ["INT", "INT", "BLOB"]
    save(filename, 'countor_constraint', types, workbook_id.value, worksheet_id.value, pickled_constraint)
    return()
